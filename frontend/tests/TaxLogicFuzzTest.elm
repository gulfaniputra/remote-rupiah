...
suite =
    describe "TaxLogic Fuzz"
        [ fuzz f4 "credit <= indoTaxDue" <| \r -> Money.toCents (cr (min r.fn r.tt) r.tt r.id r.fp) |> Expect.atMost (toFloat r.id)
        , fuzz f4 "credit <= foreignTaxPaid" <| \r -> Money.toCents (cr (min r.fn r.tt) r.tt r.id r.fp) |> Expect.atMost (toFloat r.fp)
        , fuzz f4 "credit >= 0" <| \r -> Money.toCents (cr (min r.fn r.tt) r.tt r.id r.fp) |> Expect.atLeast 0.0
        , fuzz f4 "credit <= formula cap" <| \r ->
            Money.toCents (cr (min r.fn r.tt) r.tt r.id r.fp) |> Expect.atMost (toFloat (floor (toFloat (min r.fn r.tt) * toFloat r.id / toFloat (max 1 r.tt))))
        , fuzz (Fuzz.intRange 0 999999999) "NPPN = floor(input/2)" <| \c ->
            Money.fromCents c |> TaxLogic.calculateNPPN |> Money.toCents |> Expect.equal (toFloat (c // 2))
        , fuzz (Fuzz.intRange 0 500000000000) "tax >= 0" <| \c ->
            Money.fromCents c |> TaxLogic.calculateIndoTax |> Money.toCents |> Expect.atLeast 0.0
        , fuzz (Fuzz.intRange 0 500000000000) "tax <= income" <| \c ->
            Money.fromCents c |> TaxLogic.calculateIndoTax |> Money.toCents |> Expect.atMost (toFloat c)
        , fuzz (Fuzz.intRange 1 1000000) "zero leak when actual=expected" <| \c ->
            ...
            TaxLogic.calculateFXLeakage (Money.fromCents c) 1612000 e |> Money.toCents |> Expect.equal 0.0
