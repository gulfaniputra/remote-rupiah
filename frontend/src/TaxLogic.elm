...
projectYearEndLiability g m = if m <= 0 then Money.zero else calculateIndoTax (Money.divide (Money.multiply g 12) m)
...
generateTaxReport g f = let s = aggregateAnnualSummary [{gross=g, foreignTaxPaid=f}] in { totalTaxDue = Money.toAuthoritativeString s.finalTaxPayable, proof = { nppnBasisPoints = 5000, grossIdr = Money.toAuthoritativeString s.totalGross, taxableProfitIdr = Money.toAuthoritativeString s.totalNetIncome, bracketBreakdown = [], pph24Logic = "min" } }
