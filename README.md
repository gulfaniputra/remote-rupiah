...
Developers billing U.S. clients. It computes FX conversion spreads, NPPN net
income deductions (KLU 62010), and PPh 24 foreign tax credit caps.
...
- [Architecture & API Surface](#architecture--api-surface)
- [Domain Logic & Core Guarantees](#domain-logic--core-guarantees)
...
- **FX Spread Measurement:** Surfacing conversion leakage via
  $(\text{USD Amount} \times \text{KMK Mid-Market Rate}) - \text{Actual IDR Received}$.

### Configuration (`.env`)

| Variable         | Purpose                              | Default                                                               |
| ---------------- | ------------------------------------ | --------------------------------------------------------------------- |
| `DATABASE_URL`   | PostgreSQL connection string         | `postgres://YOUR_USERNAME:YOUR_PASSWORD@localhost:5432/remote_rupiah` |
...
- Auth uses a dev-only JWT route (`ALLOW_DEV_AUTH`) instead of a prod OAuth
  flow.
- No auto-parsers for Revolut or PayPal statement imports.
- Compliance cron alerts log to stdout only with no alerting integration.
