...
- [ ] Build a personal finance and tax dashboard for an **Indonesian Individual Remote Developer
      (NPWP Pribadi)** with **100% US-based clients**.
...
- [x] **Primary Value:** Identify "monetary leaks" (FX spreads) and automate Indonesian tax
      reporting using **NPPN (Norma)** and **PPh 24 (Foreign Tax Credit)**.
...
- [ ] **Compliance Target:** DJP (Indonesia) **Coretax Portal** alignment and US-Indonesia Tax
      Treaty (W-8BEN) monitoring.
...
- [ ] **Constraint (The "Money" Rule):** \* **DO NOT USE `Float`** for currency.
  ...
  - [x] Create an **Opaque `Money` Type** in Elm wrapping a `BIGINT` (Cents/Unit).
  ...
  - [ ] Logic must reside in a pure `TaxLogic.elm` module with 100% unit test coverage (`elm-test`).
...
- [x] **KMK Rate Precision:** Every transaction must fetch the **Kurs Menteri Keuangan** valid for
      the specific week of receipt. (Note: KMK rates rotate every Wednesday).
...
- [x] **2026 Progressive Brackets:**
  - [x] 5% (0 - 60M) | 15% (60M - 250M) | 25% (250M - 500M) | 30% (500M - 5B) | 35% (> 5B).
...
- [x] The agent MUST implement the "Lesser of" rule for US Withholding (10% via W-8BEN). The allowed
      credit is the **minimum** of:
...
- [x] **Leak Calculation:** `Leak = (USD * Mid-Market Rate) - Actual IDR Received`.
...
- [ ] **Smart CSV Ingestion:** Auto-map CSVs from Wise, Revolut, Payoneer, PayPal, BCA, Mandiri, and
      BNI.
...
- [x] **Fuzzy Field Mapper:** If a CSV header is unknown, provide a UI to map fields to the internal
      schema.
...
- [x] **Unrealized Gain Tracker:** Calculate IDR gain/loss on USD balances held in foreign wallets
      (Wise/Paypal) at current market rates.
...
- [x] **Step 1:** Generate the PostgreSQL schema and Hono API boilerplate for transaction CRUD.
...
- [x] **Step 2:** Build the `Money.elm` and `TaxLogic.elm` modules. Write unit tests for the PPh 24
      cap logic.
...
- [ ] **Step 3:** Implement the KMK Rate Fetcher (Deno Cron) and the CSV Ingestion Engine.
