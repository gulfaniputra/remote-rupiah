...
Deno.test(
  "DJP Export Route - Unauthorized when Authorization header is missing",
  async () => {
...
Deno.test(
  "DJP Export Route - POST / returns 400 when body input is invalid",
  async () => {
...
Deno.test(
  "DJP Export Route - POST / returns 404 when no profile exists",
  async () => {
    testMocks.clear();
    const token = await makeToken("test-user-id-123");
    const res = await app.request("http://localhost/", {
...
Deno.test(
  "DJP Export Route - POST / returns correct Coretax CSV format and data",
  async () => {
    testMocks.clear();
    const token = await makeToken("test-user-id-123");
    ...
    // Negative adjustment/refund transaction to verify formatting fix
    testMocks.transactions.push({
      user_id: "test-user-id-123",
      date: "2026-05-20",
      amount_cents: "-105", // -$1.05
      withholding_cents: "0",
      kmk_rate: "16120.00",
      is_1042s_verified: true,
    });
    ...
    // Check negative transaction row formatting (-105n cents -> -1.05 Bruto_Valas)
    assertEquals(
      lines[3],
      "123456789012345,1234567890123456,62010,2026-05-20,-1.05,16120.00,-16926,-8463,0",
    );
  },
);
