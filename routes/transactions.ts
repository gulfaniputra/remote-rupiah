...
app.get("/", (c) =>
  withAuth(userId(c), (tx) => tx`SELECT * FROM transactions`)
    .then((txs) =>
      c.json({
        success: true,
...
app.post("/", zValidator("json", schema), async (c) => {
  ...
  const uid = userId(c);
  return withAuth(uid, (tx) =>
    tx`INSERT INTO transactions ${tx(toSnake({ ...d, kmkRate: rate, userId: uid || "" }) as Record<string, unknown>)} RETURNING *`
  ).then((res) => c.json({ success: true, data: res[0] ? serializeTx(res[0] as Record<string, unknown>) : undefined }, 201));
});

app.get("/:id", zValidator("param", z.object({ id: z.string().uuid() })), (c) =>
  withAuth(userId(c), (tx) =>
    tx`SELECT * FROM transactions WHERE id = ${c.req.valid("param").id}`
  ).then((res) =>
    res[0] ? c.json({ success: true, data: serializeTx(res[0] as Record<string, unknown>) }) : c.json({ error: "Not found" }, 404)
  )
);
...
app.patch("/:id/verify", (c) => {
  ...
  return !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)
    ...
    : withAuth(userId(c), (tx) =>
        ...
        res[0] ? c.json({ success: true, data: serializeTx(res[0] as Record<string, unknown>) }) : c.json({ error: "Fail" }, 404)
      );
});
