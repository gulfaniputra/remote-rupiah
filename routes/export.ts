...
app.post(
  ...
  zValidator(
    "json",
    z.object({
      year: z.string().regex(/^\d{4}$/).transform((v) => parseInt(v, 10)),
    }),
  ),
  (c) => {
    ...
    return withAuth(
      userId,
      (tx) =>
        tx`SELECT date, amount_cents, kmk_rate, withholding_cents FROM transactions WHERE EXTRACT(YEAR FROM date) = ${year}`,
    )
      .then((txs) =>
        !txs.length ? c.json({ error: "Empty" }, 404) : c.body(
          new ReadableStream({
            start(ctrl) {
              ctrl.enqueue(
                new TextEncoder().encode(
                  "Bulan,Bruto_Valas,Kurs_KMK,Bruto_IDR,Netto_IDR,PPh_24_Kredit_IDR\n",
                ),
              );
              ...
              for (const t of txs) {
                ...
                ctrl.enqueue(
                  new TextEncoder().encode(
                    ...
                    },${t.amount_cents},${t.kmk_rate},${g},${n},${w}\n`,
                  ),
                );
              }
              ctrl.enqueue(
                new TextEncoder().encode(`TOTAL,,,${tg},${tn},${tw}\n`),
              );
              ctrl.close();
            },
          }),
          200,
          {
            ...
            "Content-Disposition": `attachment; filename="SPT_${year}.csv"`,
          },
        )
      );
  },
