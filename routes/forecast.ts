...
app.get("/", c => {
  ...
  return withAuth((c.get as (key: string) => unknown)("userId") as string | undefined, (tx) => tx`
    SELECT
    ...
    .then(([r]) => c.json({ success: true, forecast: { year, ytdGrossCents: r.ytd_gross_cents, ytdWithholdingCents: r.ytd_withholding_cents, ytdActualIdrCents: r.ytd_actual_idr_cents, transactionCount: r.transaction_count, verifiedCount: r.verified_count, avgFxSpreadCents: r.avg_fx_spread_cents, latestMonth: r.latest_month || 0 } }));
});
