...
const readBodyWithLimit = async (body: ReadableStream<Uint8Array> | null, maxBytes: number) => {
  if (!body) return null;
...
app.post("/preview", async (c) => {
  try {
    ...
    if (ct.includes("multipart/form-data")) {
      ...
      if (!size || size > MAX_PAYLOAD_BYTES) return c.json({ success: false, error: "Payload too large (max 5MB)" }, 413);
    ...
    if (ct.includes("text/csv") || ct.includes("text/plain")) {
      ...
      if (body === null) return c.json({ success: false, error: "Payload too large (max 5MB)" }, 413);
...
app.post("/", async (c) => {
  try {
    const body = await readBodyWithLimit(c.req.raw.body, MAX_PAYLOAD_BYTES);
    if (body === null) return c.json({ success: false, error: "Payload too large" }, 413);
