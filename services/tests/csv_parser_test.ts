...
Deno.test("headers", () => assertEquals(parseCsvStream(CSV).headers, ["Date", "Description", "Amount", "Currency", "Fee"]));
...
Deno.test("row data", () => { const r = parseCsvStream(CSV).rows[0]; assertEquals(r["Amount"], "1,250.55"); assertEquals(r["Currency"], "USD"); });
Deno.test("total count", () => assertEquals(parseCsvStream(CSV).totalRowCount, 6));
Deno.test("empty", () => assertEquals(parseCsvStream(""), { headers: [], rows: [], totalRowCount: 0 }));
Deno.test("header-only", () => { const r = parseCsvStream("Date,Amount\n"); assertEquals(r.headers, ["Date", "Amount"]); assertEquals(r.rows, []); });
