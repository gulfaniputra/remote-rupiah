 ...
 */
const parseMandiriDate = (raw: string): string => {
  const trimmed = raw.trim();

  // Try DD/MM/YYYY first (standard Mandiri format)
  ...
  throw new Error(`Invalid Mandiri transaction date: "${raw}"`);
};

/**
 * Extract the amount from a Mandiri row.
 *
 * Mandiri CSV format uses separate "Debet" (outgoing) and "Kredit" (incoming) columns.
 * Also supports a single "Jumlah" column for simplified exports.
 *
 * Returns the amount in cents (positive for incoming, negative for outgoing).
 */
const extractAmount = (r: Record<string, string>): bigint => {
  // Format 1: Separate Debet/Kredit columns
  ...
  throw new Error(
    `Cannot determine amount from Mandiri row: no Debet, Kredit, or Jumlah column found`,
  );
};

export const mapMandiriRow = (r: Record<string, string>) => {
  const dateRaw = r["Tanggal"] || r["Date"] || "";
  const date = parseMandiriDate(dateRaw);
