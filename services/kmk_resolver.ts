 ...
 */
export async function getKmkRateByDate(date: Date): Promise<{ effective_date: string; rate: string }> {
  const effectiveDate = resolveKmkWeek(date);

  // QA Check: Always Wednesday, Never future date
  if (effectiveDate.getUTCDay() !== 3) throw new Error("Resolved KMK date must be a Wednesday");
  if (effectiveDate > date) throw new Error("Resolved KMK date cannot be in the future");
